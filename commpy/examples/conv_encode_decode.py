...
# g_matrix = np.array((1011011, 1111001), ndmin=2)
...
# %%
# Modem : QPSK
modem = mod.QAMModem(4); KMod = 1/np.sqrt(2)
# SNR range to test
SNRs = np.arange(0, 6) + 10 * math.log10(modem.num_bits_symbol)
print(f"modem.num_bits_symbol {modem.num_bits_symbol}")
...
# Modulation function
def modulate(bits):
    return KMod*modem.modulate(cc.conv_encode(bits, trellis1, 'cont'))
...
BERs_soft = model_soft.link_performance(SNRs, 100, 600, 5000, code_rate)
